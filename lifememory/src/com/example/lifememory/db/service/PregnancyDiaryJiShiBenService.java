...
public class PregnancyDiaryJiShiBenService {
	private SQLiteDatabase db = null;

	public PregnancyDiaryJiShiBenService(Context context) {
	 ...
	 */
	public void addPregnancyDiaryJiShiBen(PregnancyJiShiBen item) {
		db.execSQL(
				"insert into diary_jishiben (title, content, textcolorindex, textsizeindex, createdate, createymd, createym, ismodyfied, imageid) values (?, ?, ?, ?, ?, ?, ?, ?, ?)",
	...
	//根据createdate修改数据
	public void updateItemByCreateDate(PregnancyJiShiBen item) {
		db.execSQL("update diary_jishiben set title = ?, content = ?, textcolorindex = ?, textsizeindex = ? where createdate = ?", new String[]{item.getTitle(), item.getContent(), 
				String.valueOf(item.getTextColorIndex()), String.valueOf(item.getTextSizeIndex()), item.getCreateDate()});
	}

	// 按时间降序查找所有记事本日记
	public ArrayList<PregnancyJiShiBen> listAll() {
		ArrayList<PregnancyJiShiBen> jishibenItems = new ArrayList<PregnancyJiShiBen>();
		PregnancyJiShiBen item = null;
		Cursor cursor = db.rawQuery(
				"select * from diary_jishiben order by createdate desc", null);
		while (cursor.moveToNext()) {
			item = new PregnancyJiShiBen();
	 ...
	 * @param item
	 */
	public void updateItem(PregnancyJiShiBen item) {
		db.execSQL(
				"update diary_jishiben set  content = ? , textcolorindex = ? , textsizeindex = ?, updatedate = ?, updateymd = ?, updateym = ?, ismodyfied = 1 where idx = ?",
	...
	// 查找记事本
	public PregnancyJiShiBen findItemById(int id) {
		PregnancyJiShiBen item = new PregnancyJiShiBen();
		Cursor cursor = db.rawQuery(
				"select * from diary_jishiben where idx = ?",
	...
	public void updateTitleById(int idx, String title, String updateDate,
			String updateYmd) {
		db.execSQL(
				"update diary_jishiben set title = ?, updatedate = ?, updateymd = ?, ismodyfied = 1 where idx = ?",
	...
	// 查找今天创建的日记本
	public ArrayList<PregnancyJiShiBen> getTodayItems(String todayYmd) {
		ArrayList<PregnancyJiShiBen> jishibenItems = new ArrayList<PregnancyJiShiBen>();
		PregnancyJiShiBen item = null;
		Cursor cursor = db.rawQuery(
				"select * from diary_jishiben where  createymd = ? or updateymd = ? ",
	...
	public List<String>  getMonths(String todayym) {
		...
		String month = null;
		Cursor cursor = db.rawQuery("select createym from diary_jishiben group by createym", null);
		while(cursor.moveToNext()) {
	...
	// 今天的，本周，本月，每月
	// yyyy年MM月dd
	public List<PregnancyJiShiBen> findItemsByYMD(String todayymd) {
		 List<PregnancyJiShiBen> jishibenItems = new ArrayList<PregnancyJiShiBen>();
		 PregnancyJiShiBen item = null;
		 Cursor cursor = db.rawQuery("select * from diary_jishiben  where createymd = ? order by createdate desc", new
		 String[]{todayymd});
		 while(cursor.moveToNext()) {
				item = new PregnancyJiShiBen();
				item.setIdx(cursor.getInt(cursor.getColumnIndex("idx")));
	...
	// 今天的，本周，本月，每月
	// yyyy年MM月dd
	public List<PregnancyJiShiBen> findItemsByYM(String todayym, String todayYMD) {
		 List<PregnancyJiShiBen> jishibenItems = new ArrayList<PregnancyJiShiBen>();
		 PregnancyJiShiBen item = null;
		 Cursor cursor = db.rawQuery("select * from diary_jishiben  where createym = ? and createymd != ? order by createdate desc", new
	...
	public void deleteItemsByIdxs(List<Integer> idxs) {
		if(idxs.size() > 0) {
			...
			sb.append(")");
//			Log.i("a", sb.toString());
			String sqlStr = "delete from diary_jishiben where idx in " + sb.toString();
			db.execSQL(sqlStr);
	...
	public void deleteAll() {
		db.execSQL("delete from diary_jishiben");
	}
