...
public class BabyLuYinAddActivity extends Activity {
    ...
    private Handler mHandler = new Handler() {
    	public void handleMessage(android.os.Message msg) {
    		...
    		if(msg.what == 0) {
    			mediaStop(true);
    			BabyLuYinAddActivity.this.finish();
...
//			Toast.makeText(getApplicationContext(), "rename", 0).show();
			if(isRecodePage) {
				//录音界面
				new DialogPregnancyJiShiBenReNameDialog(BabyLuYinAddActivity.this, listener2, "").show();
			}else {
				new DialogPregnancyJiShiBenReNameDialog(BabyLuYinAddActivity.this, listener2, titleTv.getText().toString()).show();
			}
			break;
		case R.id.btn2:
			//播放暂停按钮
...
//			CopyFileFromData.getInstance().copyDatabase(BabyLuYinAddActivity.this, "pregnancy_diary.db");
